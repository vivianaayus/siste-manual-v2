...
Debido al aumento en el n\'umero de los par\'ametros, los modelos puedan explicar mejor los datos, lo que no implica que los modelos sean \emph{\textbf{m\'as} realistas, por lo que la selecci\'on de un modelo no puede basarse en que el modelo mejore la probabilidad de los datos, sino si la mejora es (o no es) estad\'isticamente significativa, dado el n\'umero de par\'ametros usados en el modelo.
...
En general se puede usar \Pname{JModeltest}, o \Pname{PAUP*}, o \Pname{R + ape + PhyML} o \Pname{JModeltest + PhyML}. 
 
%\footnote{Si por alg\'un motivo no desea usar \Pname{Java} o prefiere tener m\'as control sobre los c\'alculos iniciales de los modelos, una alternativa puede ser  \Pname{PAUP* + Modeltest} o \Pname{MrModeltest}, que es una modificaci\'on de \Pname{Modeltest} pero calcula un menor n\'umero de modelos; en caso que de usar \Pname{MrModeltest}, tenga en mente que \textbf{no} ha evaluado todos los modelos.} 
% no creo que valga lapenausar esta nota
...
\begin{enumerate}		
	...
	\item use 
	   ...
	   para hacer una b\'usqueda con el equivalente a 3 modelos en \Pname{jmodeltest}, compare losresultadoscon lab\'usquedaanterior.
