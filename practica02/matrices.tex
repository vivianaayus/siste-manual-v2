...
Abra la tabla de datos morfol\'ogicos para vertebrados \Datos{datos.vertebrados.xls} con un programa para hojas de c\'alculo y manuipule las matrices con \Pname{WinClada}, \Pname{Mesquite} o \Pname{TNT} para Windows:
...
\begin{enumerate}
	...
	\item Construya una matriz nueva (con \Pname{TNT} para Windows use el menu \Gui{Data - edit data}). 
	...
	\item Exporte la matriz en otro formato, \Fname{NEXUS} si est\'a usando \Pname{WinClada}, \Fname{Hennig86/Nona} si est'a usando \Pname{Mesquite}.
	...
	\begin{itemize}
		...
		\item Carge las bibliotecas \Rlib{ape} y \Rlib{phangorn}\footnote{La biblioteca \Rlib{phangorn} est\'a diseñada para an\'alisis filogen\'etico que tiene como objetivo estimar \'arboles filogen\'eticos y redes,  utilizando diferentes m\'etodos como m\'axima verosimilitud,  parsimonia,  distancia y conjugaci\'on de Handamard. Requiere las bibliotecas $"$ape$"$ y $"$rgl$"$,  y puede ser descargado desde \url {http://cran.r-project.org/web/packages/phangorn/index.html}}: 
		\Cmd{library(ape)}
		...
		\Cmd{datosPhylip $<-$ read.phyDat(''DNA1.phy'', format=''phylip'', type=''DNA'')} 

		\item Escriba en formato  \Fname{Nexus} la matriz leida anteriomente:
		\Cmd{write.nexus.data(datosPhylip,file=''NuevaDNA1.nex'')}
